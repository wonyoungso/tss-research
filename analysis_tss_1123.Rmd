...
# with covariates - all
model_2 <- glm(data = tss_df_centered, formula = yesno ~ view_mode * score +  nou_2_4_units_cent + nou_4_10_units_cent + nou_10_50_units_cent + nou_50_units_cent + race_asian_cent + race_black_cent + race_hispanic_cent + race_native_cent + race_other_cent + gender_female_cent, family = "binomial")
...
stargazer(model_3_rob, model_4_rob, type = "text",
          add.lines = list(c("neg2ll", round(c(model_3$deviance, model_4$deviance), 2)),
                           c("r2pseudo", round(unlist(lapply(list(model_3, model_4), FUN = function(x) 1 - x$deviance/x$null.deviance)), 4))), star.char = c("†", "*", "**", "***"),
...
stargazer(model_5_rob, model_6_rob, type = "text",
          add.lines = list(c("neg2ll", round(c(model_5$deviance, model_6$deviance), 2)),
                           c("r2pseudo", round(unlist(lapply(list(model_5, model_6), FUN = function(x) 1 - x$deviance/x$null.deviance)), 4))), star.char = c("†", "*", "**", "***"),
          star.cutoffs = c(0.1, 0.05, 0.01, 0.001),
          notes = c("† p<0.1; * p<0.05; ** p<0.01; *** p<0.001"), 
          notes.append = F)
...
model_7 <- lm(security_deposit ~ view_mode * score, data = tss_df_sd)
model_7_rob <- coeftest(model_7, vcov. = vcovCL(model_7,  cluster=tss_df_sd$user_id, type ="HC2"))

model_8 <- lm(security_deposit ~ view_mode * score + income_linear_cent + okay_blanket_policy_yes_cent + nou_2_4_units_cent + nou_4_10_units_cent + nou_10_50_units_cent + nou_50_units_cent + race_asian_cent + race_black_cent + race_hispanic_cent + race_native_cent + race_other_cent + gender_female_cent, data = tss_df_sd)
model_8_rob <- coeftest(model_8, vcov. = vcovCL(model_8,  cluster=tss_df_sd$user_id))


stargazer(model_8_rob, type="text", star.char = c("†", "*", "**", "***"),
          star.cutoffs = c(0.1, 0.05, 0.01, 0.001),
          notes = c("† p<0.1; * p<0.05; ** p<0.01; *** p<0.001"), 
          notes.append = F)
